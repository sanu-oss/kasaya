...
const evaluateTest = response => {
  ...
  const isValidTestCase = flatResponse.find(
    atomicResponse =>
      atomicResponse === ASSERTION.PASS || atomicResponse === ASSERTION.FAIL
  );

  if (isValidTestCase) {
    return flatResponse.find(
      atomicResponse => atomicResponse === ASSERTION.FAIL
    )
      ? TEST.FAIL
      : TEST.PASS;
  }
...
const emitTestSummary = testCases => {
  if (testCases) {
    const passingCount = testCases.filter(testCase => testCase === TEST.PASS)
      .length;
