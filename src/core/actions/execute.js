  ...
  if (browser) {
    if (functionIdentifier) {
      ...
      if (stringArgument) {
        ...
        parsedArgumentArray = argumentArray
          ? argumentArray.map(argument => {
      ...
      const executeFunctionStatus = await browser.execute(
        functionExecutor,
        functionIdentifier,
        parsedArgumentArray
      );
      if (executeFunctionStatus === undefined) {
        return logger.emitLogs({
          message: `Function "${functionIdentifier}" was not found on the browser!`,
          type: MESSAGE_TYPE.ERROR
        });
      }
      if (executeFunctionStatus === null) {
        return logger.emitLogs({
          message: BROWSER_EXECUTE_FUNC_ERR,
          type: MESSAGE_TYPE.ERROR
        });
      }
      return logger.emitLogs({
        message: `Function "${functionIdentifier}" executed successfully on the browser.`,
