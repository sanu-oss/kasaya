...
module.exports = async (
  ...
  if (browser) {
    ...
    const doMouseAction = async element => {
      ...
      try {
        ...
        return logger.emitLogs({
          message: `Cannot perform the mouse action on '${selectorValue}', maybe use 'inspect' to check whether you are clicking on a valid element.`,
      ...
      if (
        ...
        if (crossElements.success) {
          if (crossElements.targetResults.length === 1) {
            ...
            crossElements.targetResults.length > 1 &&
            !Number.isNaN(Number(elementIndex)) &&
            crossElements.targetResults.length > Number(elementIndex)
          ) {
            await browser.execute(eraseHighlights);
            const element = await browser.$(
              crossElements.targetResults[elementIndex]
            );
            await doMouseAction(element);
          }
        }
    ...
    if (success === false) {
      switch (code) {
        case 'TARGET_NOT_FOUND': {
          return logger.emitLogs({
            message: `Could not locate an element matching text "${selectorValue}"`,
            type: MESSAGE_TYPE.ERROR
          });
        }
    ...
    if (
      ...
      !Number.isNaN(Number(elementIndex)) &&
      targetResults.length > Number(elementIndex)
    ) {
