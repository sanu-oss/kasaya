...
const selectiveRead = async (
  state,
  { isSelector, isTextField, isPlaceholder },
  { args: { selector, marker, elementIndex, destinationVar } }
) => {
  ...
  const retrieveInputValue = async inputElement => {
    ...
    if (element) {
      ...
      logger.emitLogs({
        message: `Read tokens as: "{"${destinationVar.replace(
          '$',
          ''
        )}":"${value}"}"`,
...
const activeRead = async (state, { args: { destinationVar } }) => {
  const browser = validateBrowser(state);
