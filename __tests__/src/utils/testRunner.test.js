...
describe('emitTestLog test suite', () => {
  test('passing test log', () => {
    ...
    expect(messageEmitter.emitTestSuccess).toHaveBeenCalledWith(
      `${mockInput.status} :: ${mockInput.command}`
    );
  });
  ...
  test('failing test log', () => {
    ...
    emitTestLog(mockInput);
    expect(messageEmitter.emitTestWarn).toHaveBeenCalledWith(
      `${mockInput.status} :: ${mockInput.command}`
    );
  });
});
...
describe('emitVerboseLog test suite', () => {
  test('passing test log should log test status', () => {
    ...
    expect(messageEmitter.emitPlainText).toHaveBeenCalledWith(
      `[-] ${mockInput.status} :: ${mockInput.command}\n`
    );
  });
});
