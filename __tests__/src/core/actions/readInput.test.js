...
} = require('../../../../src/utils/browser/nearestElementFinder');
const {
  activeRead,
  selectiveRead
} = require('../../../../src/core/actions/readInput');
...
describe('read input commands test suite', () => {
  test('read input command should log the status of the checkbox when isSelector is true', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: true, targetResults: xpathArray });

    await selectiveRead(
      state,
      { isSelector: true },
      { args: { selector: selectorText, destinationVar } }
    );
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: 'Read tokens as: "{"subject":"true"}"',
  ...
  test('read input command should log the value of the input field when isTextField is true', async () => {
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: 'Read tokens as: "{"subject":"input_value"}"',
      type: MESSAGE_TYPE.INFO
    });
  });
  ...
  test('read input command should log the placeholder value when isPlaceholder is true', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: true, targetResults: xpathArray });
    when(state.browser.execute)
      .calledWith(getPlaceholder, element)
      .mockResolvedValue('input_value');
  ...
  test('read input command should log the closest input field value to the given element when the elementFinder gives a non input element', async () => {
    ...
    const state = {
      browser: {
        ...
        getElementText: jest
          .fn()
          .mockResolvedValue('Hello users, welcome to Kasaya')
      }
    };
    logger.emitLogs = jest.fn();
    store.setGlobal = jest.fn();
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: true, targetResults: xpathArray });
    when(state.browser.execute)
      .calledWith(nearestElementFinder, element, 'INPUT')
      .mockResolvedValue('some_xpath');
  ...
  test('read input command should log the value of the input field when the elementIndex is given to select one from the multiple matches', async () => {
    ...
    await selectiveRead(
      state,
      { isTextField: true },
      { args: { selector: selectorText, destinationVar, elementIndex } }
    );
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: 'Read tokens as: "{"subject":"input_value"}"',
      type: MESSAGE_TYPE.INFO
  ...
  test('read input command should log an error message when the search text is not found', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: false, code: 'TARGET_NOT_FOUND' });
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: `Could not locate an element matching text "${selectorText}"`,
  ...
  test('read input command should log an error message when the marker text is not found', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: false, code: 'BASE_ELEMENT_NOT_FOUND' });

    await selectiveRead(
      state,
      { isTextField: true },
      { args: { selector: selectorText, marker: markerText, destinationVar } }
    );
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: `Could not locate an element matching text "${markerText}"`,
      type: MESSAGE_TYPE.ERROR
    });
  });
  ...
  test('read input command should log an error message when multiple markers are found', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: false, code: 'MULTIPLE_BASE_ELEMENTS' });
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: `Found more than one elements matching text "${markerText}". Try using a unique text after 'near' keyword.`,
      type: MESSAGE_TYPE.ERROR
    });
  });
  ...
  test('read input command should log an error message when element finder gives an unknown error', async () => {
    ...
    when(state.browser.execute)
      ...
      .mockResolvedValue({ success: false, code: 'UNKNOWN_ERROR' });
...
describe('read active field commands test suite', () => {
  test('read active field should log the active field value', async () => {
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: 'Read tokens as: "{"subject":"resolved_value"}"',
      type: MESSAGE_TYPE.INFO
    });
  });
});
