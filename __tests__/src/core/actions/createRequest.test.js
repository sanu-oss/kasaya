...
describe('createRequest command test suite ', () => {
  test('createRequest action should be able to create a get request to a specified endpoint and store response data', async () => {
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: `Initialized variable as: "${varName}: ${JSON.stringify(
        users
      )}"`,
  ...
  test('createRequest action should emit a specific error message if unable to fetch data', async () => {
    const method = 'get';
    const url = 'http://testUrl';
    const varName = '$testVar';
  ...
  test('createRequest action should emit an error message if varName does not start with a "$"', async () => {
    const method = 'get';
    const url = 'http://testUrl';
    const varName = 'testVar';
