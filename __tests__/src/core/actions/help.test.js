...
describe('help command test suite', () => {
  ...
  test('help action should display all the commands when "help" is typed', async () => {
    ...
    expect(messageEmitter.emitInfo).toHaveBeenCalledWith(
      'These are common Kasāya commands used in various situations:'
    );
    expect(messageEmitter.emitHelp).toHaveBeenCalledWith(
      'greet John - Greets a specified person',
      maxCommandLength
    );
    expect(messageEmitter.emitHelp).toHaveBeenCalledWith(
      'run hello - Print "Hello"',
      maxCommandLength
    );
  });
  ...
  test('help action should display all the commands when command specific help is triggered', async () => {
    ...
    expect(messageEmitter.emitInfo).toHaveBeenCalledWith(
      "Here are the matching help entries for 'greet':"
    );
    expect(messageEmitter.emitHelp).toHaveBeenCalledWith(
      'greet John - Greets a specified person',
      maxCommandLength
