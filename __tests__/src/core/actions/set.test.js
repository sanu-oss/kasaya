...
describe('set command test suite', () => {
  test('copy function should set the value to new key', async () => {
    ...
    when(store.setGlobal)
      .calledWith({ key: newKey, value: valueInStore })
      .mockReturnValue(true);

    await set.copyValue({
      args: { sourceVar: storeKey, destinationVar: newKey }
    });
    ...
    expect(logger.emitLogs).toHaveBeenCalledWith({
      message: `Initialized variable as: "${newKey}: ${valueInStore}"`,
